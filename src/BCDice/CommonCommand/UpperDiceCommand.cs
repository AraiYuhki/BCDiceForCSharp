...
namespace BCDice.CommonCommand
{
    ...
    /// 上方ダイス（ボーナスダイス）コマンド
    /// 複数のダイスを振り、最大値を採用する
    /// 例: 2B6 → 2つのD6を振り、大きい方を採用
    /// </summary>
    public class UpperDiceCommand : ICommonCommand
    {
        /// <summary>
        /// シングルトンインスタンス
        /// </summary>
        public static readonly UpperDiceCommand Instance = new UpperDiceCommand();
        ...
        /// <inheritdoc/>
        public string PrefixPattern => @"\d+[BU]";
        ...
        public Result? Eval(string command, IGameSystemContext gameSystem, IRandomizer randomizer)
        {
            ...
            int maxValue = rolls.Max();

            var sb = new StringBuilder();
            sb.Append($"({count}B{sides})");
            ...
            if (match.Groups[3].Success && match.Groups[4].Success)
            {
                ...
                if (compareOp.HasValue)
                {
                    success = Compare(maxValue, compareOp.Value, target);
