...
namespace BCDice.GameSystem
{
    ...
    /// デモンパラサイト
    /// </summary>
    public sealed class DemonParasite : GameSystemBase
    {
        /// <summary>
        /// シングルトンインスタンス
        /// </summary>
        public static readonly DemonParasite Instance = new DemonParasite();

        private static readonly Regex UrgeRegex = new Regex(
            @"^([NAMUC])?URGE\s*(\d+)$",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);

        /// <inheritdoc/>
        public override string Id => "DemonParasite";

        /// <inheritdoc/>
        public override string Name => "デモンパラサイト";

        /// <inheritdoc/>
        public override string SortKey => "てもんはらさいと";
        ...
        {
            return GetUrge(command, randomizer);
        }

        private Result? GetUrge(string str, IRandomizer randomizer)
        {
            var m = UrgeRegex.Match(str);
            if (!m.Success)
            {
                return null;
            }

            var initialWord = m.Groups[1].Success ? m.Groups[1].Value.ToUpperInvariant() : null;
            var urgelv = int.Parse(m.Groups[2].Value);
            ...
            {
                return Result.CreateBuilder("衝動段階は1から5です").Build();
            }

            var diceNow = randomizer.RollSum(2, 6);
            var resultText = urge[urgelv - 1][diceNow - 2];

            return Result.CreateBuilder($"{title}{urgelv}-{diceNow}:{resultText}").Build();
        }

        private static readonly string[][] URGE_TABLE = new[]
        {
            new[]
            {
        ...
        private static readonly string[][] NEW_URGE_TABLE = new[]
        {
            new[]
            {
        ...
        private static readonly string[][] MALFUNCTION_TABLE = new[]
        {
            new[]
            {
        ...
        private static readonly string[][] MUTANT_TABLE = new[]
        {
            new[]
            {
        ...
        private static readonly string[][] ONIMITAMA_OUT_OF_BATTLE_TABLE = new[]
        {
            new[]
            {
        ...
        private static readonly string[][] ONIMITAMA_BATTLE_TABLE = new[]
        {
            new[]
            {
